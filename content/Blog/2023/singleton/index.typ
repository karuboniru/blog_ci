...
- New entries is always added #link("https://github.com/bminor/glibc/blob/d1dcb565a1fb5829f9476a1438c30eccc4027d04/stdlib/cxa_atexit.c#L79-L141")[to the end of the list]
- When a global object #link("https://github.com/bminor/glibc/blob/d1dcb565a1fb5829f9476a1438c30eccc4027d04/stdlib/cxa_atexit.c#L32-L70")[is done constructed, an entry will be added] to the `exit_function_list` structure to register the destructor of the object.
...
- For objects initialized after `_dl_fini` is registered, the destructors will be called in the reverse order of initialization, this is the ordinary order as we expected and stated in #link("https://en.cppreference.com/w/cpp/utility/program/exit")[std::exit()]

- `_dl_fini` is a bit tricky since it will #link("https://github.com/bminor/glibc/blob/d1dcb565a1fb5829f9476a1438c30eccc4027d04/elf/dl-fini.c#L91-L94")[do a resort of loaded shared libraries], by the order that one will be in front of those it depends on. After the sort, `_dl_call_fini` for each of the shared library #link("https://github.com/bminor/glibc/blob/d1dcb565a1fb5829f9476a1438c30eccc4027d04/elf/dl-fini.c#L114")[will be called in the order of the sorted list].

- `_dl_call_fini` calls `__cxa_finalize` with the address of `__dso_handle` of given shared library as argument. `__cxa_finalize` also iterates through the `exit_function_list` structure and call the destructors in the reverse order of initialization, but #link("https://github.com/bminor/glibc/blob/d1dcb565a1fb5829f9476a1438c30eccc4027d04/stdlib/cxa_finalize.c#L25-L27")[it will only select the destructors that are registered with the given `__dso_handle`] in this case. So `_dl_call_fini` will only destory local static objects that initialized from `anything::get_instance()` inside the shared library.
