...
+ 在表格、block、list等环境用 `#footnote` 生成的旁注对齐错误 #footnote[
    ...
    这一条比较复杂，很大程度上包括两个更改：
    + 修改 CSS 规则保证在各种环境下 `footnote` 的位置不会错的太离谱（之前在table中调用 `footnote` 结果是旁注被塞到了表格里面）
    + 用一个 JS 脚本在页面加载时把所有的 `footnote` 元素移动横向位置正确对齐（纵向位置不变）
  ]
...
- 出于我的喜好添加了 tags 功能，依旧是 python 脚本扫描所有 `typ` 文件的路径，传递给一个专门的 `typ` 文件，其中使用Typst原生方法扫描元数据，构建出按照tag分类的文章列表。
